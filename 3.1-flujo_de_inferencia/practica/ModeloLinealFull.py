...
def _update(X, y, mu, S, alpha, beta):
    ...
    beta += float(0.5 * (mu_0.T.dot(S_0.dot(mu_0)) + y.T.dot(y) - mu.T.dot(S.dot(mu))))
...
def _model_evidence(
    N, S_N, alpha_N, beta_N, S_0=None, alpha_0=None, beta_0=None, log=True
):
    ...
    if S_0 is not None:
        ...
        D = 0.5 * np.log(np.linalg.det(S_N)) - 0.5 * np.log(np.linalg.det(S_0))
...
class BayesianLinearModel(object):
    ...
    def __init__(
        self, basis, D=None, location=None, dispersion=None, shape=None, scale=None
    ):
    ...
    def __initialise(self, D=None):
        ...
        # Check that the location parameter is an array.
            ...
            not isinstance(self.__mu_0, np.ndarray)
            or self.__mu_0.ndim != 2
        ...
        # data (after basis function expansion).
        elif (self.__S_0.shape[0] != self.__D) and (self.__S_0.shape[1] != self.__D):
    ...
    def empirical_bayes(self, x0, X, y):
        ...
        self.__mu_N, self.__S_N, self.__alpha_N, self.__beta_N = _uninformative_fit(
            phi, y
        )
    ...
    def update(self, X, y):
        ...
        self.__mu_N, self.__S_N, self.__alpha_N, self.__beta_N = _update(
            phi, y, self.__mu_N, self.__S_N, self.__alpha_N, self.__beta_N
        )
    ...
    def predict(self, X, y=None, variance=False):
        ...
        if (y is not None) or variance:
            S_hat = _predict_variance(phi, self.__S_N, self.__alpha_N, self.__beta_N)
            ...
            # Return mean, 95% confidence interval and likelihood.
                ...
                # Return array.
                if N == K:
                    q = _posterior_likelihood(
                        y.squeeze(), m_hat.squeeze(), S_hat.squeeze(), self.__alpha_N
                    )
    ...
    def random(self, samples=1):
        ...
        r = scipy.stats.invgamma.rvs(
            self.__alpha_N, scale=self.__beta_N, size=samples
        ).reshape((1, 1, samples))
